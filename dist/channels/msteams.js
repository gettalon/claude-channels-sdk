...
export function parseConfig() {
    ...
    if (!appId)
        throw new Error("TEAMS_APP_ID is required");
    if (!appPassword)
        throw new Error("TEAMS_APP_PASSWORD is required");
...
// ─── Channel Factory ─────────────────────────────────────────────────────────
export async function createTeamsChannel(config) {
    // @ts-ignore - botbuilder is an optional peer dependency
    const _botbuilder = await import("botbuilder");
    const { BotFrameworkAdapter, ActivityHandler, CardFactory, MessageFactory, TurnContext } = _botbuilder;
    ...
    const server = http.createServer(async (req, res) => {
        ...
        req.on("end", async () => {
            ...
            try {
                ...
                await adapter.process(mockReq, res, async (context) => {

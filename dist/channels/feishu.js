...
// ─── Channel Factory ─────────────────────────────────────────────────────────
export async function createFeishuChannel(config) {
        ...
        "im.message.receive_v1": async (data) => {
            ...
            try {
                ...
                if (msgType === "text") {
                    ...
                    const firstLang = Object.values(post)[0];
                    if (firstLang?.content) {
                        text = firstLang.content
                            .flat()
                            .map((el) => el.text ?? "")
                            .join("");
                    }
        ...
        req.on("end", async () => {
            ...
            try {
                ...
                if (parsed.header?.event_type) {
                    ...
                    const handler = eventDispatcher.handlers?.[eventType];
                    if (handler) {

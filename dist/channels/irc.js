...
export function parseConfig() {
    ...
    if (!nick)
        throw new Error("IRC_NICK is required");
    if (channels.length === 0)
        throw new Error("IRC_CHANNELS is required (comma-separated)");
                ...
                if (Buffer.from(test, "utf-8").length > limit) {
                    ...
                    if (Buffer.from(word, "utf-8").length > limit) {
                        ...
                        while (Buffer.from(remaining, "utf-8").length > limit) {
                            ...
                            let end = limit;
                            while (Buffer.from(remaining.slice(0, end), "utf-8").length > limit)
                                end--;
...
// ─── Channel Factory ─────────────────────────────────────────────────────────
export async function createIrcChannel(config) {
    ...
    const pendingPermissions = new Map(); // request_id -> nick or channel

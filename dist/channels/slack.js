...
export function parseConfig() {
    ...
    const appToken = cfg.slackAppToken();
...
// ─── Channel Factory ─────────────────────────────────────────────────────────
export async function createSlackChannel(config) {
    ...
    app.action(/^perm:.+$/, async ({ action, ack, respond }) => {
        await ack();
    ...
    channel.onToolCall(async (name, args) => {
        switch (name) {
            ...
            case "slack_upload": {
                ...
                const filename = args.filename ?? filePath.split("/").pop() ?? "file";
                const title = args.title;
                const initialComment = args.initial_comment;

...
export function installRouting(Hub) {
    ...
    Hub.prototype.resolvedName = function () {
        return this.clientConnected() ? (this.opts.agentName ?? this.name) : this.name;
    };
    /** Remove a chat route and its associated channel entry, then persist. */
    Hub.prototype.clearRoute = function (chatId) {
        this.chatRoutes.delete(chatId);
        this.channelForChat.delete(chatId);
        this.persistState().catch(() => { });
    };
    /** Emit a chat message to host and fire hooks. */
    Hub.prototype.emitMessage = function (content, chatId, user, extra) {
     ...
     */
    Hub.prototype.resolveTarget = function (target) {
    ...
    Hub.prototype.trySendToRoute = function (chatId, content, from, rich) {
    ...
    Hub.prototype.sendMessage = function (target, content, rich) {
        if (!target)
        ...
        if (knownChannel) {
            this.wsSendAsync(knownChannel.ws, { type: "chat", chat_id: resolved, content, from: myName, ...rich });
            return { ok: true };
        }
        // Proxy to connected hub peers first — they may have this agent locally
        // (must come before channel fallback to avoid misrouting agent UUIDs to channels like Telegram)
    ...
    Hub.prototype.reply = function (chatId, text, rich) {
        const myName = this.resolvedName();
        ...
        // Proxy to connected hub peers before falling back to channel clients
        const hubPeers = [...this.clients.values()].filter(c => c.role === "server");
        if (hubPeers.length > 0) {
            const msgId = rich?.msgId ?? randomUUID();
            for (const peer of hubPeers) {
    ...
    Hub.prototype.routeChat = function (params) {
        ...
        if (from && chatId) {
            const channelType = source === "channel"
                ? (this.channelForChat.get(chatId)?.transport ?? this.inferChannelType(chatId))
                : "agent";
        ...
        const botNames = [];
        const botUsername = this.opts?.botUsername;
        if (botUsername)
        ...
        if (agentMention) {
            ...
            if (targetAgent) {
                ...
                if (senderAgentId) {
                    ...
                    if (authErr) {
                        ...
                        if (senderAgent)
                            this.wsSend(senderAgent.ws, { type: "ack", ref: "agent_auth", status: "error", reason: authErr });
            ...
            if (this._persistentAgentRouter) {
                const routed = this._persistentAgentRouter(mentionedName, restContent.trim() || content, from, chatId);
                if (routed) {
            ...
            if (routeOwner === senderAgentId) {
                ...
                if (channelClient) {
                    ...
                    if (authError) {
                        ...
                        if (agent)
                            this.wsSend(agent.ws, { type: "ack", ref: "channel_auth", status: "error", reason: authError });
     ...
     */
    Hub.prototype.wrapEnvelope = function (payload, opts) {
        const myName = this.resolvedName();
     ...
     */
    Hub.prototype.route = function (envelope, filter = "broadcast") {
        let delivered = 0;
        ...
        if (sendToAgents) {
            if (filter === "session-scoped") {
                ...
                if (envelope.to) {
                    ...
                    if (routedId)
                        targetAgent = this.agents.get(routedId);
                }
                if (targetAgent) {
    ...
    Hub.prototype.handover = function (chatId, toAgentId) {
        if (this.isClient()) {
            return this.proxyToServer("handover", { chatId, toAgentId });
        }
    ...
    Hub.prototype.getChatRoute = function (chatId) {
        if (this.isClient()) {
            return this.proxyToServer("getChatRoute", { chatId });
        }
        return this.chatRoutes.get(chatId);
    };
    /** Release a chat assignment, returning it to the host. */
    Hub.prototype.releaseChat = function (chatId) {
        if (this.isClient()) {
            return this.proxyToServer("releaseChat", { chatId });
        }
        if (!this.chatRoutes.has(chatId))
...
//# sourceMappingURL=hub-routing.js.map
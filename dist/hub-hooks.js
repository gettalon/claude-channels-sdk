...
export function installHooks(Hub) {
     ...
     */
    Hub.prototype.addShellHook = async function (event, command) {
     ...
     */
    Hub.prototype.loadShellHooks = async function () {
        const settings = await this.loadSettings();
        if (settings.hooks?.length) {
            for (const h of settings.hooks) {
                // Avoid duplicates if already loaded
                const exists = this.shellHooks.some((s) => s.event === h.event && s.command === h.command);
                if (!exists) {
                    this.shellHooks.push({ event: h.event, command: h.command });
                }
            }
     ...
     */
    Hub.prototype.fireHooks = async function (event, ...args) {
        // Global hooks toggle — skip all hooks when disabled
        if (this._hooksEnabled === false)
            return;
        // Programmatic hooks
        ...
        if (fns) {
            for (const fn of fns) {
                try {
                    ...
                    process.stderr.write(`[${this.name}] Hook error (${event}): ${e}\n`);
                }
            }
        }
        // Shell command hooks
     ...
     */
    Hub.prototype.execShellHook = function (command, event, args) {
        return new Promise((resolve) => {
            ...
            exec(command, { env, timeout: 30_000 }, (err, stdout, stderr) => {
                ...
                if (stdout)
                    process.stderr.write(`[${this.name}] Hook stdout (${event}): ${stdout.trim()}\n`);
                if (stderr)
                    process.stderr.write(`[${this.name}] Hook stderr (${event}): ${stderr.trim()}\n`);
                resolve();
            });
        });
    };
}
//# sourceMappingURL=hub-hooks.js.map
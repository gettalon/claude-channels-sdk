...
export function installClient(Hub) {
    Hub.prototype.connect = async function (url, agentName, connectionConfig) {
        ...
        const name = agentName ?? this.opts.agentName ?? HubConfigService.fromEnv().envAgentName ?? this.name ?? randomAgentName();
        ...
        if (actualTransport === "telegram" && !transportConfig.botToken) {
            transportConfig.botToken = HubConfigService.fromEnv().telegramBotToken();
        }
        const adapter = createChannel(actualTransport, transportConfig);
        const transport = await adapter.connect(resolvedUrl, (msg) => {
                ...
                if (msg.status === "pending_approval") {
                    ...
                    if (msg.info) {
                        (async () => {
                            try {
                                const settings = await this.loadSettings();
                                const conn = (settings.connections ?? []).find((c) => c.url === storeUrl || c.url === url);
                                if (conn) {
            ...
            if (msg.type === "tool_call")
                this.handleClientToolCall(msg, transport);
            ...
            if (msg.type === "proxy_result") {
                ...
                const p = this.pendingProxyCalls.get(msg.proxy_id);
                if (p) {
                    clearTimeout(p.timer);
                    this.pendingProxyCalls.delete(msg.proxy_id);
                    if (msg.error)
        ...
        const ws = {
            send: (data) => transport.send(JSON.parse(data)),
            get readyState() {
        ...
        if (storedClient)
            storedClient.name = displayName;
        // Auto-register in unified target registry
        if (this.registerTarget) {
            this.registerTarget(displayName, actualTransport, channelId, "channel");
        }
        ...
        await transport.send({ type: "register", agent_name: name, tools: this.clientTools, metadata: publicKey ? { publicKey } : undefined });
        // Heartbeat
        heartbeatTimer = setInterval(() => {
            ...
            transport.send({ type: "heartbeat" }).catch(() => { });
        }, 15000);
        const client = this.clients.get(storeUrl);
        if (client)
    ...
    Hub.prototype.handleClientToolCall = async function (msg, transport) {
        const toolArgs = typeof msg.args === "string" ? JSON.parse(msg.args) : (msg.args ?? {});
        try {
            let result;
            if (this.opts.onToolCall) {
                result = await this.opts.onToolCall(msg.tool_name, toolArgs);
            }
            else
                result = { error: `Unknown tool: ${msg.tool_name}` };
            await transport.send({ type: "tool_result", call_id: msg.call_id, result });
        }
        catch (e) {
            await transport.send({ type: "tool_result", call_id: msg.call_id, error: String(e) });
        }
    };
}

...
export async function createArchitectServer(opts = {}) {
    ...
    hub._persistentAgentRouter = (name, content, from, chatId) => {
    ...
    try {
        ...
        architectInstructions = readFileSync(tplPath, "utf-8")
            .replace(/\{\{port\}\}/g, String(hub.opts?.port ?? 9090))
            .replace(/\{\{owner_id\}\}/g, "");
    }
    catch { }
    // ── MCP Server ─────────────────────────────────────────────────────────
    const mcp = new Server({ name: serverName, version: opts.version ?? "1.0.0" }, {
        capabilities: {
    ...
    mcp.oninitialized = () => {
        if (mcpReady) {
            ...
            const parts = [];
            if (!isClientOnly) {
                for (const [, srv] of hub.servers ?? new Map()) {
                    if (srv.type === "websocket")
                        parts.push(`Server :${srv.port}`);
                }
            }
            for (const c of (hub.clients ? [...hub.clients.values()] : [])) {
                if (c.channel && c.channel !== "unix")
                    parts.push(`${c.name ?? c.id} via ${c.channel}`);
            }
    ...
    hub.on("agentConnected", ({ id, name, tools }) => {
        ...
        notify("notifications/claude/channel", {
            content: `Agent "${name}" connected with tools: ${tools.map((t) => t.name).join(", ") || "none"}`,
    ...
    let cachedMaxInline = null;
    hub.loadSettingsSafe().then(s => { cachedMaxInline = s.maxInlineLength ?? 500; }).catch(() => { cachedMaxInline = 500; });
    ...
    hub.hook("onMessage", async ({ content, user }) => {
        ...
        if (atMentions?.length) {
            for (const m of atMentions) {
                ...
                if (target !== user)
                    hub.sendMessage(target, `[from ${user}] ${cleanContent}`);
            }
        }
        ...
        if (tildeRefs?.length) {
            for (const m of tildeRefs) {
                ...
                if (blocker !== user)
                    hub.sendMessage(blocker, `[waiting] ${user} is blocked by you: ${cleanContent}`);
            }
        }
    });
    hub.on("message", async ({ content, chatId, user, type, source, files }) => {
        ...
        const displayChatId = chatId ? hub.displayName(chatId) : "unknown";
    ...
    hub.on("approvalGranted", ({ agentId, url, info }) => {
        ...
        const remoteServerName = info?.server_name ?? url;
    ...
    mcp.setNotificationHandler(PermReqSchema, async ({ params }) => {
        for (const agent of hub.agents.values())
            hub.wsSend(agent.ws, { type: "permission_request", request: params });
    });
    // ── Tool registry ───────────────────────────────────────────────────────
    ...
    try {
        ...
        const socketInUse = existsSync(socketPath) && await new Promise((resolve) => {
            const sock = createConnection({ path: socketPath }, () => { sock.destroy(); resolve(true); });
            sock.on("error", () => resolve(false));
            sock.setTimeout(500, () => { sock.destroy(); resolve(false); });
        });
        const portInUse = socketInUse || await new Promise((resolve) => {
                ...
                if (existingPid && existingPid !== process.pid) {
                    ...
                    if (isAlive) {
                        ...
                        try {
                            ...
                            process.stderr.write(`[${serverName}] Killed stale architect (PID ${existingPid})\n`);
...
export async function createAgentMcpServer(opts = {}) {
    ...
    const mcp = new Server({ name: agentName, version: opts.version ?? "1.0.0" }, {
        capabilities: {

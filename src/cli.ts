...
interface BackendDef {
  ...
  defaultArgs: string[];
  /** How to inject a prompt into the args */
  promptArg: (prompt: string) => string[];
  /** Whether the tool reads prompts from stdin (interactive REPL) */
  interactive: boolean;
}
...
function parseArgs(argv: string[]): { backend: string; args: string[]; prompt: string | null } {
...
async function main(): Promise<void> {
  ...
  const def: BackendDef = BACKENDS[backend] ?? {
    command: backend,
    defaultArgs: [],
    promptArg: (p) => [p],
    interactive: true,
  ...
  const activeChatId: { value: string | null } = { value: null };

 ...
 */
export function deriveMeshId(secret: string): string {
 ...
 */
export function createMeshJwt(secret: string, deviceId: string, ttlSeconds = 3600): string {
 ...
 */
export function verifyMeshJwt(token: string, secret: string): MeshJwtPayload | null {
  ...
  const meshId = (raw.mesh_id ?? raw.meshId) as string | undefined;
  const devId = (raw.device_id ?? raw.deviceId) as string | undefined;
  const iat = raw.iat as number | undefined;
  const exp = raw.exp as number | undefined;

  if (!meshId || !devId || typeof iat !== "number" || typeof exp !== "number") return null;
 ...
 */
export async function loadOrCreateIdentity(talonHome?: string): Promise<IdentityStore> {
  ...
  try {
    ...
    const identity: IdentityStore = { ...kp, createdAt: new Date().toISOString() };
 ...
 */
export function deriveEncryptionKey(secret: string, salt: string = "mesh-e2e-key"): Buffer {
...
export class E2eSession {
  ...
  private constructor(key: Buffer, deviceId: string) {
    this.key = key;
...
// ── Sender Keys (Signal-style group E2E) ─────────────────────────────────────
//
// Each group member generates a random sender key (AES-256).
// The sender key is distributed to all other members via pairwise E2E channels.
// When sending to the group, the member encrypts once with their own sender key.
// All members who have received the key can decrypt. The hub only sees ciphertext.
...
export interface SenderKeyDistribution {
  ...
  bundles: Array<{ to: string; encrypted: EncryptedPayload }>;
...
export class SenderKeySession {
  ...
  private ownKey: Buffer;
  private ownKeyHex: string;
  private chainIndex: number;
  private deviceId: string;
  private group: string;

  /** Sender keys received from other members: memberId → { key, chainIndex } */
  private peerKeys = new Map<string, { key: Buffer; chainIndex: number }>();
  ...
  distribute(pairwiseSessions: Map<string, E2eSession>): SenderKeyDistribution {
    const bundles: Array<{ to: string; encrypted: EncryptedPayload }> = [];
  ...
  receiveDistribution(dist: SenderKeyDistribution, ourSession: E2eSession): void {
    ...
    const bundle: SenderKeyBundle = JSON.parse(decrypted);
  ...
  decrypt(msg: SenderKeyEncryptedMessage): string {
    ...
    if (!peer) throw new Error(`No sender key for "${msg.from}"`);
    if (peer.chainIndex !== msg.chainIndex) throw new Error(`Chain index mismatch: expected ${peer.chainIndex}, got ${msg.chainIndex}`);
...
export class MeshDiscovery extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    this.mdns.on("query", (query: any) => {
      const isOurs = query.questions?.some(
        (q: any) => q.name === MDNS_SERVICE_TYPE || q.name?.endsWith("._talon-mesh._tcp.local"),
      );
      if (!isOurs) return;
    ...
    this.mdns.on("response", (response: any) => {
      const srvRecord = response.answers?.find(
        (a: any) => a.type === "SRV" && a.name?.includes("_talon-mesh"),
      );
      const txtRecord = response.answers?.find(
        (a: any) => a.type === "TXT" && a.name?.includes("_talon-mesh"),
      );
      ...
      if (txtRecord?.data) {
        const txtEntries: string[] = Array.isArray(txtRecord.data)
          ? txtRecord.data.map((d: any) => (typeof d === "string" ? d : d.toString()))
          : [txtRecord.data.toString()];
        for (const entry of txtEntries) {
          if (entry.startsWith("meshId=")) meshId = entry.slice(7);
          if (entry.startsWith("deviceId=")) deviceId = entry.slice(9);
        }
      }

      // Only accept peers from the same mesh
      const ourMeshId = deriveMeshId(this.config.meshSecret);
      if (meshId && meshId !== ourMeshId) return;

      // Derive a peer IP from additional records if available
      const aRecord = response.additionals?.find((a: any) => a.type === "A");
      ...
      if (peer.name === selfName && peer.id === (this.config.deviceId ?? selfName)) return;
  ...
  stop(): void {
    ...
    if (this.mdns) {
      try { this.mdns.destroy(); } catch { /* ignore */ }
      this.mdns = null;
    }
    this.peers.clear();
  }
...
export class MeshRegistry {
   ...
   */
  startReporting(info: { lan?: Array<{ ip: string; port: number }> }): void {
    const report = async () => {
      try {
 ...
 */
export function parseMeshConfig(env: Record<string, string | undefined>): MeshConfig | undefined {
  if (!env.MESH_SECRET) return undefined;
  return {

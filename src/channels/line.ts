...
export function parseConfig(): LineConfig {
...
export async function createLineChannel(
  ...
  const line = await import("@line/bot-sdk") as any;
  ...
  function handleWebhookEvents(events: any[]): void {
    for (const event of events) {
      if (event.type !== "message") continue;
      if (!("message" in event) || event.message.type !== "text") continue;

      const textMsg = event.message as any;
      const text = textMsg.text;
      const userId = event.source?.userId ?? "";
      const groupId = event.source?.type === "group" ? (event.source as any).groupId : "";
      const roomId = event.source?.type === "room" ? (event.source as any).roomId : "";
      const chatId = groupId || roomId || userId;
      const replyToken = ("replyToken" in event ? event.replyToken : "") as string;

...
export async function createDiscordChannel(
  ...
  const { Client, GatewayIntentBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, ChannelType } = _discord as any;

  const cfg = { ...parseConfig(), ...config };
  if (!cfg.token) throw new Error("DISCORD_TOKEN is required");
  ...
  const pendingPermissions = new Map<string, string>(); // request_id -> channelId
  ...
  channel.onPermissionRequest(async (req: ChannelPermissionRequest) => {
    ...
    for (const channelId of targets) {
      try {
        ...
        const sent = await (ch as any).send({ content, components: [row] });
  ...
  channel.onToolCall(async (name: string, args: Record<string, unknown>) => {
    switch (name) {
      case "discord_react": {
        ...
        const ch = await client.channels.fetch(channelId);
        if (!ch || !("messages" in ch)) throw new Error("Channel not found or not text-based");
      ...
      case "discord_thread": {
        ...
        const autoArchiveDuration = (args.auto_archive_duration as number) ?? 1440;

        const ch = await client.channels.fetch(channelId);
        if (!ch || !("threads" in ch)) throw new Error("Channel not found or not a text channel");
    ...
    if (input.hook_event_name === "Notification" && "message" in input) {
      ...
      for (const channelId of targets) {
        try {
          const ch = await client.channels.fetch(channelId);
          if (ch && "send" in ch) {
            await (ch as any).send(`**[Notification]** ${input.message}`);
          }
        } catch {}
      }
    }

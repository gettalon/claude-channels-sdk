...
export async function createIMessageChannel(
  ...
  async function checkChatDbAccess(): Promise<boolean> {
    ...
    try {
      ...
      process.stderr.write(
        `[imessage] Cannot read chat.db (likely SIP/TCC restriction). ` +
  ...
  send "${escapedText}" to targetBuddy
      ...
      try {
        ...
        throw new Error(
          `Failed to send iMessage to ${to}: ${fallbackErr.message}. ` +
  ...
  async function pollNewMessages(): Promise<void> {
    if (!chatDbAvailable || shuttingDown) return;
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (isNaN(rowId) || !text || !handleId) continue;
        ...
        await channel.pushMessage(
          `<channel source="imessage" chat_id="${chatId}" user="${handleId}" service="${service}">\n${text}\n</channel>`,
          { chat_id: chatId, user: handleId, service },
        );
      }
    } catch (err: any) {
      if (!shuttingDown) {
  ...
  async function queryContacts(limit: number): Promise<Array<{ handle: string; service: string; lastMessage: string }>> {
    if (!chatDbAvailable) {
      return [];
    }
  ...
  async function queryHistory(contact: string, limit: number): Promise<Array<{ text: string; isFromMe: boolean; date: string }>> {
    if (!chatDbAvailable) {
      return [];
    }
  ...
  if (!chatDbAvailable) {
    instructions.push(
      `WARNING: chat.db is not accessible. Incoming message polling is disabled. ` +

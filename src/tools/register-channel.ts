...
export const registerChannelTool: ToolDefinition = {
  ...
  handle: async (args, ctx) => {
    ...
    if (mod) {
      try {
        ...
        if (typeof factory === "function") { const { registerChannel } = await import("../protocol.js"); registerChannel(type, factory); }
        else return JSON.stringify({ error: `No factory in ${mod}` });
      } catch (e) { return JSON.stringify({ error: `Import failed: ${e}` }); }
    }

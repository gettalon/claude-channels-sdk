...
export const callToolTool: ToolDefinition = {
  name: "call_tool",
  ...
  handle: async (args, ctx) => {
    const agentIdOrName = args.agent_id as string;
    ...
    if (agent) {
      try { return JSON.stringify(await ctx.hub.callRemoteTool(agent.id, args.tool_name as string, toolArgs), null, 2); }
    ...
    if (ctx.hub.isClient()) {
      try {
        const agents = await ctx.hub.listAgents() as Array<{ id: string; name: string }>;
        const match = agents.find(a => a.id === agentIdOrName || a.name === agentIdOrName);
        if (match) {
          return JSON.stringify(await ctx.hub.callRemoteTool(match.id, args.tool_name as string, toolArgs), null, 2);
        }
      } catch (e) { return JSON.stringify({ error: String(e) }); }
    }

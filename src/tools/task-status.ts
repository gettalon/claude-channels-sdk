...
export const taskStatusTool: ToolDefinition = {
  ...
  handle: async (args) => {
    ...
    const filter = (args.agent as string | undefined)?.toLowerCase();
    const statusFilter = (args.status as string | undefined)?.toLowerCase();
    let results: string[] = [];
    let totalTasks = 0;
    ...
    try {
      ...
      for (const dir of dirs) {
        if (dir.startsWith(".") || dir.includes(".json")) continue;
        const jsonPath = joinPath(agentsDir, dir, "agent.json");
        let data;
        try { data = JSON.parse(readFileSync(jsonPath, "utf-8")); } catch { continue; }
        ...
        for (const t of tasks) {
          if (statusFilter && (t.status || "pending") !== statusFilter) continue;
          totalTasks++;

/**
 * hub-health.ts — Health monitor and health snapshot for ChannelHub.
 * Extracted from hub.ts (lines 1732–1840).
 */
import type { ChannelHub, HealthSnapshot } from "./hub.js";

/** Install health monitor methods onto the ChannelHub prototype. */
export function installHealth(Hub: typeof ChannelHub): void {

  /** Return a point-in-time health snapshot (no side effects). */
  Hub.prototype.getHealth = async function(this: ChannelHub): Promise<HealthSnapshot> {
    const serverResults: HealthSnapshot["servers"] = [];
    ...
    const clientResults: HealthSnapshot["clients"] = [];
    ...
    for (const a of this.agents.values()) {
      if (now - a.lastHeartbeat > 90000) stale++;
    }
   ...
   */
  Hub.prototype.startHealthMonitor = function(this: ChannelHub, intervalMs = 30000): void {
    if ((this as any).healthMonitorTimer) return; // already running
    ...
    (this as any).healthMonitorTimer = setInterval(runCheck, intervalMs);
    // Run the first check immediately
    runCheck();
  };

  /** Stop the health monitor if running. */
  Hub.prototype.stopHealthMonitor = function(this: ChannelHub): void {
    if ((this as any).healthMonitorTimer) {
      clearInterval((this as any).healthMonitorTimer);
      (this as any).healthMonitorTimer = null;
    }
  };
}

...
function checkChannelAuth(hub, agentId, channelTransport) {
    const agent = hub.agents.get(agentId);
    if (!agent || !agent.allowedChannels?.length)
        return null;
    if (agent.allowedChannels.includes(channelTransport))
        return null;
...
function checkAgentAuth(hub, senderAgentId, targetAgentId) {
...
export class HubRouter {
    ...
    // ── Registry ──────────────────────────────────────────────────────────
    registerTarget(name, channelType, rawId, kind, sourceUrl) {
    ...
    findTarget(nameOrUuid) {
        ...
        if (uuid)
            return this.hub.targetRegistry.get(uuid);
    ...
    displayName(chatId) {
        ...
        const client = [...this.hub.clients.values()].find((c) => c.channelId === chatId || c.url?.includes(chatId));
        if (client?.name)
            return client.name;
        return chatId;
    }
    resolvedName() {
        return this.hub.clientConnected() ? (this.hub.opts?.agentName ?? this.hub.name) : this.hub.name;
    }
    // ── Route management ──────────────────────────────────────────────────
    clearRoute(chatId) {
        this.hub.chatRoutes.delete(chatId);
        this.hub.channelForChat.delete(chatId);
        this.hub.persistState?.().catch(() => { });
    }
    getChatRoute(chatId) {
        if (this.hub.isClient()) {
            return this.hub.proxyToServer("getChatRoute", { chatId });
        }
        return this.hub.chatRoutes.get(chatId);
    }
    releaseChat(chatId) {
        if (this.hub.isClient()) {
            return this.hub.proxyToServer("releaseChat", { chatId });
        }
        if (!this.hub.chatRoutes.has(chatId))
            return { ok: false, error: "No route for this chat" };
        this.clearRoute(chatId);
        process.stderr.write(`[${this.hub.name}] Chat "${chatId}" released back to host\n`);
        return { ok: true };
    }
    // ── Messaging ─────────────────────────────────────────────────────────
    emitMessage(content, chatId, user, extra) {
        this.hub.emit("message", { content, chatId, user, type: "chat", ...extra });
        this.hub.fireHooks?.("onMessage", { content, chatId, user, type: "chat", ...extra })?.catch?.(() => { });
    }
    resolveTarget(target) {
        ...
        const contact = this.hub.resolveContact?.(target);
        if (contact) {
    ...
    trySendToRoute(chatId, content, from, rich) {
    ...
    sendMessage(target, content, rich) {
        if (!target)
            return { ok: false, error: "Target required" };
        const myName = this.resolvedName();
        const resolved = this.resolveTarget(target);
        if (this.trySendToRoute(resolved, content, myName, rich))
            return { ok: true };
        ...
        if (knownChannel) {
            this.hub.wsSendAsync(knownChannel.ws, { type: "chat", chat_id: resolved, content, from: myName, ...rich });
            return { ok: true };
        }
        for (const [, client] of this.hub.clients) {
            if (client.role === "channel") {
                this.hub.wsSendAsync(client.ws, { type: "chat", chat_id: resolved, content, from: myName, ...rich });
                return { ok: true };
            }
        }
        const hubPeers = [...this.hub.clients.values()].filter((c) => c.role === "server");
        if (hubPeers.length > 0) {
            const msgId = rich?.msgId ?? randomUUID();
    ...
    reply(chatId, text, rich) {
        const myName = this.resolvedName();
        const resolved = this.resolveTarget(chatId);
        if (this.trySendToRoute(resolved, text, myName, rich))
            return { ok: true };
        const agent = this.hub.findAgent(resolved);
        if (agent) {
        ...
        if (targetEntry?.sourceUrl) {
            ...
            if (sourceClient) {
                ...
                this.hub.wsSendAsync(sourceClient.ws, replyPayload);
                return { ok: true };
            }
        }
        ...
        if (knownChannel) {
            ...
            this.hub.wsSendAsync(knownChannel.ws, replyPayload);
            return { ok: true };
        }
        for (const [, client] of this.hub.clients) {
            if (client.channelId?.includes(resolved) || client.url?.includes(resolved)) {
                ...
                this.hub.wsSendAsync(client.ws, replyPayload);
                return { ok: true };
            }
        }
        for (const [, client] of this.hub.clients) {
    ...
    routeChat(params) {
        ...
        if (from && chatId) {
            const channelType = source === "channel"
                ? (this.hub.channelForChat.get(chatId)?.transport ?? this.hub.inferChannelType?.(chatId))
                : "agent";
            this.hub.autoRegisterContact?.(from, chatId, channelType);
        }
        if (source === "channel" && chatId && !this.hub.channelForChat.has(chatId)) {
            const channelClient = (sourceUrl ? this.hub.clients.get(sourceUrl) : undefined)
                ?? [...this.hub.clients.values()].find((c) => c.role === "channel" && (!sourceUrl || c.url === sourceUrl))
                ?? [...this.hub.clients.values()].find((c) => c.role === "channel");
            if (channelClient)
                this.hub.channelForChat.set(chatId, channelClient);
        }
        // 1. Strip bot/hub mention
        const botNames = [];
        const botUsername = this.hub.opts?.botUsername;
        if (botUsername)
            botNames.push(botUsername);
        ...
        if (agentMention) {
            ...
            if (targetAgent) {
                if (senderAgentId) {
                    ...
                    if (authErr) {
                        ...
                        if (senderAgent)
                            this.hub.wsSend(senderAgent.ws, { type: "ack", ref: "agent_auth", status: "error", reason: authErr });
                ...
                if (!this.hub.chatRoutes.has(chatId)) {
                    ...
                    const channelClient = this.hub.channelForChat.get(chatId) ?? (sourceUrl ? this.hub.clients.get(sourceUrl) : undefined) ?? [...this.hub.clients.values()].find((c) => c.role === "channel");
                    if (channelClient)
                        this.hub.channelForChat.set(chatId, channelClient);
                }
            ...
            if (this.hub._persistentAgentRouter) {
                const routed = this.hub._persistentAgentRouter(mentionedName, restContent.trim() || content, from, chatId);
                if (routed) {
        ...
        if (!this.hub.chatRoutes.has(chatId)) {
            ...
            if (bestAgent) {
                ...
                const channelClient = this.hub.channelForChat.get(chatId) ?? [...this.hub.clients.values()].find((c) => c.role === "channel");
                if (channelClient)
                    this.hub.channelForChat.set(chatId, channelClient);
            ...
            if (routeOwner === senderAgentId) {
                ...
                if (channelClient) {
                    ...
                    if (authError) {
                        ...
                        if (agent)
                            this.hub.wsSend(agent.ws, { type: "ack", ref: "channel_auth", status: "error", reason: authError });
        ...
        // ChatRoute dispatch
        if (this.trySendToRoute(chatId, content, from))
            return;
        // Broadcast to host
        this.emitMessage(content, chatId, from);
    }
    // ── Envelope & EventRouter ────────────────────────────────────────────
    wrapEnvelope(payload, opts) {
        const myName = this.resolvedName();
        const msgType = payload?.type;
        let envelopeType = "system";
        if (msgType === "chat" || msgType === "reply")
            envelopeType = "chat";
        else if (msgType === "tool_call" || msgType === "tool_call_proxy")
            envelopeType = "tool_call";
        else if (msgType === "tool_result")
            envelopeType = "tool_result";
        else if (msgType === "invite" || msgType === "group_broadcast")
            envelopeType = "handover";
        return createEnvelope(myName, envelopeType, payload, opts);
    }
    route(envelope, filter = "broadcast") {
        let delivered = 0;
        ...
        if (sendToAgents) {
            if (filter === "session-scoped") {
                ...
                if (envelope.to) {
                    ...
                    if (routedId)
                        targetAgent = this.hub.agents.get(routedId);
                }
                if (targetAgent) {
    ...
    // ── Chat Handover ─────────────────────────────────────────────────────
    handover(chatId, toAgentId) {
        if (this.hub.isClient()) {
            return this.hub.proxyToServer("handover", { chatId, toAgentId });
        }
        const agent = this.hub.findAgent(toAgentId);
        if (!agent)
            return { ok: false, error: "Agent not found" };
        const channelClient = [...this.hub.clients.values()].find((c) => c.channelId === chatId || c.channelId.endsWith(`:${chatId}`));
...
//# sourceMappingURL=hub-router.js.map
...
import type { HubSettings, GroupMember } from "@gettalon/hub-core";

// ── Settings path & lock state ────────────────────────────────────────────

// Canonical source for TALON_HOME resolution — HubConfigService.talonHome delegates here.
// process.env is intentionally read here (and only here) rather than via HubConfigService,
// since this module is the one that feeds HubConfigService.
...
/** Install settings + state persistence methods onto a hub class prototype. */
export function installSettings(Hub: any): void {
  Hub.prototype.loadSettings = function(this: any): Promise<HubSettings> {
    return loadSettings();
  };

  Hub.prototype.loadSettingsSafe = function(this: any): Promise<HubSettings> {
    return loadSettingsSafe();
  };

  Hub.prototype.saveSettings = function(this: any, settings: HubSettings): Promise<void> {
    return saveSettings(settings);
  };

  Hub.prototype.registerServer = function(this: any, url: string, name: string, port: number): Promise<void> {
    return registerServer(url, name, port);
  };

  Hub.prototype.unregisterServer = function(this: any, port: number): Promise<void> {
    return unregisterServer(port);
  };

  Hub.prototype.getRegisteredServers = function(this: any): Promise<HubSettings["servers"]> {
    return getRegisteredServers();
  };

  Hub.prototype.addConnection = function(this: any, url: string, name: string, config?: Record<string, unknown>): Promise<void> {
    return addConnection(url, name, config);
  };

  Hub.prototype.removeConnection = function(this: any, url: string): Promise<void> {
    return removeConnection(url);
  };

  Hub.prototype.getConnections = function(this: any): Promise<HubSettings["connections"]> {
    return getConnections();
  };

  Hub.prototype.persistState = async function(this: any): Promise<void> {
    const newState = {
      chatRoutes: Object.fromEntries(
        [...this.chatRoutes.entries()].map(([chatId, agentId]: [string, string]) => {
      ...
      groups: Object.fromEntries(
        [...this.groups.entries()].map(([name, memberMap]: [string, Map<string, GroupMember>]) => [name, [...memberMap.values()].map(m => m.mode === "all" ? m.name : `${m.name}|${m.mode}`)])
      ),
      targets: Object.fromEntries(
        [...(this.targetRegistry as Map<string, any>).entries()]
          .filter(([, entry]: [string, any]) => entry.kind !== "agent") // agents reconnect with new IDs — don't persist
          .map(([uuid, entry]: [string, any]) => [uuid, { name: entry.name, channelType: entry.channelType, rawId: entry.rawId, kind: entry.kind, ...(entry.sourceUrl ? { sourceUrl: entry.sourceUrl } : {}) }])
      ),
    };
    const json = JSON.stringify(newState);
    if (json === this.lastPersistedStateJson) return;
  ...
  Hub.prototype.restoreState = async function(this: any): Promise<void> {
    const settings = await loadSettings();
    if (settings.state?.chatRoutes) {
      for (const [chatId, info] of Object.entries(settings.state.chatRoutes)) {
        ...
        const agent = this.findAgent((info as any).agentName);
        if (agent) {
    ...
    if (settings.state?.targets) {
      for (const [uuid, entry] of Object.entries(settings.state.targets)) {
        if (this.registerTarget) {
          this.registerTarget((entry as any).name, (entry as any).channelType, (entry as any).rawId, (entry as any).kind, (entry as any).sourceUrl);
        }
      }

...
async function ensureTalonDir(): Promise<void> {
...
export async function daemonStatus(): Promise<DaemonStatus> {
  ...
  if (pid !== null && !running) await removePid();
  return { running, pid: running ? pid : null, pidFile: PID_FILE, logFile: LOG_FILE };
}

// ── Stop ─────────────────────────────────────────────────────────────────────

export async function daemonStop(): Promise<{ stopped: boolean; pid: number | null; error?: string }> {
  const pid = await readPid();
  if (pid === null) return { stopped: false, pid: null, error: "No PID file found — daemon not running" };
  if (!isAlive(pid)) {
    await removePid();
...
export async function daemonStart(opts: { port?: number } = {}): Promise<DaemonStartResult> {
  ...
  // Clean stale
  if (existing !== null) await removePid();
  ...
  const child: ChildProcess = spawn(process.execPath, [thisFile, ...args], {
    detached: true,
...
export async function daemonRestart(opts: { port?: number } = {}): Promise<DaemonStartResult> {
  await daemonStop();
...
async function runWorker(): Promise<void> {
  ...
  // Dynamic import — the root package provides ChannelHub.
  // Use variable to avoid TypeScript resolving the circular workspace dependency.
  const sdkPath = "@gettalon/channels-sdk";
  const { ChannelHub } = await import(/* webpackIgnore: true */ sdkPath) as any;
  ...
  if (telegramToken) {
    const hasTelegram = [...hub.clients.values()].some((c: any) => c.transport === "telegram");
    if (!hasTelegram) {
  ...
  let activeSession: ChildProcess | null = null;

  hub.on("message", ({ content, chatId, user, type }: any) => {
    if (type !== "chat") return;
    ...
    if (!activeSession || activeSession.exitCode !== null) {
      ...
      let response = "";
      activeSession.stdout?.on("data", (chunk: Buffer) => { response += chunk.toString(); });
...
<dict>
  ...
  <dict>
    ...
    } catch (e) { return { enabled: false, path: PLIST_PATH, error: String(e) }; }
  } else {
    // Linux systemd
    const unit = `[Unit]
...
export async function daemonDisable(): Promise<{ disabled: boolean; error?: string }> {
  try {
    if (process.platform === "darwin") {
      const { execSync } = await import("node:child_process");
  ...
  } catch (e) { return { disabled: false, error: String(e) }; }
}

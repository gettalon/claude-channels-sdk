...
export class HubClientRuntime {
    ...
    // ── Main connect method ────────────────────────────────────────────────
    async connect(url, agentName, connectionConfig) {
        const hub = this.hub;
        ...
        const findExisting = () => {
            if (hub.clients.has(url))
                return hub.clients.get(url);
            if (port) {
                const aliases = [`auto://localhost:${port}`, `ws://localhost:${port}`, `unix:///tmp/talon-${port}.sock`];
                for (const alias of aliases) {
                    if (alias !== url && hub.clients.has(alias))
                        return hub.clients.get(alias);
                }
            }
            return undefined;
        };
        ...
        if (existing) {
            ...
            try {
                existing.ws.send(JSON.stringify({ type: "register", agent_name: name, tools: hub.clientTools }));
            }
            catch { }
            return;
        }
        ...
        await transport.send({ type: "register", agent_name: name, tools: hub.clientTools, metadata: publicKey ? { publicKey } : undefined });
        // Heartbeat
    ...
    async handleClientToolCall(msg, transport) {
        const hub = this.hub;
        const toolArgs = typeof msg.args === "string" ? JSON.parse(msg.args) : (msg.args ?? {});
        try {
            let result;
            if (hub.opts.onToolCall) {
                result = await hub.opts.onToolCall(msg.tool_name, toolArgs);
            }
            else
...
//# sourceMappingURL=hub-client-runtime.js.map
...
async function transcribeAudio(audioBuffer, cohereApiKey) {
...
async function cohereTranscribe(audioBuffer, apiKey) {
...
class TelegramTransport {
    ...
    async send(message) {
        ...
        if (msg.type === "chat" || msg.type === "reply") {
            ...
            const replyMarkup = msg.buttons?.length ? {
                inline_keyboard: [msg.buttons.map((b) => b.url ? { text: b.text, url: b.url } : { text: b.text, callback_data: b.action ?? b.text })]
            } : undefined;
            ...
            if (msg.meta?.poll_question && msg.meta?.poll_options) {
                try {
                    const options = JSON.parse(msg.meta.poll_options);
            ...
            if (msg.files?.length) {
                ...
                if (msg.files.length > 1 && msg.files.every((f) => f.data && (f.mime?.startsWith("image/") || f.mime?.startsWith("video/")))) {
                    try {
                        const media = msg.files.map((f) => ({
                            type: f.mime?.startsWith("image/") ? "photo" : "video",
                ...
                for (const file of msg.files) {
                    if (file.path) {
                        ...
                        try {
                            ...
                            if (msg.caption || text)
                                form.append("caption", msg.caption ?? text);
                        ...
                        if (!res.ok)
                            process.stderr.write(`[telegram] File send failed: ${await res.text().catch(() => res.status)}\n`);
                    }
                    else if (file.url) {
    ...
    pickSendMethod(mime, name) {
        if (!mime)
            return "sendDocument";
        if (mime === "image/gif" || mime === "video/gif")
        ...
        if (mime.startsWith("audio/"))
            return "sendAudio";
        if (mime === "video/mp4" && name?.includes("video_note"))
    ...
    async editMessage(messageId, text, parseMode) {
    ...
    /** Send a location */
    async sendLocation(chatId, latitude, longitude, replyTo) {
    ...
    /** Send a venue */
    async sendVenue(chatId, latitude, longitude, title, address, replyTo) {
    ...
    /** Send a poll */
    async sendPoll(chatId, question, options, isAnonymous = true, allowsMultiple = false) {
    ...
    async sendAnimation(chatId, animation, caption, fileName) {
        const form = new FormData();
        form.append("chat_id", chatId);
    ...
    async sendVideoNote(chatId, videoNote) {
        const form = new FormData();
        form.append("chat_id", chatId);
    ...
    handleIncoming(text, meta) {
        ...
        const chatMsg = { type: "chat", chat_id: this.chatId, content: text, from: fromName };
        const msgMeta = {};
        let hasMeta = false;
        if (meta?.voice) {
            msgMeta.voice = "true";
            if (meta.duration != null)
                msgMeta.duration = String(meta.duration);
            if (meta.file_id)
                msgMeta.file_id = meta.file_id;
            hasMeta = true;
        }
        if (meta?.reply_to_message) {
            const reply = meta.reply_to_message;
            if (reply.message_id != null) {
                msgMeta.reply_to_id = String(reply.message_id);
                hasMeta = true;
            }
            const replyFrom = reply.from?.username ?? reply.from?.first_name;
            if (replyFrom) {
                msgMeta.reply_to_user = replyFrom;
                hasMeta = true;
            }
            if (reply.text) {
                msgMeta.reply_to_text = reply.text.slice(0, 200);
                hasMeta = true;
            }
...
export class TelegramAdapter {
    ...
    token;
    polling = false;
    pollGeneration = 0; // incremented on each startPolling to invalidate old loops
    transports = new Map(); // chatId → transport
    connectionHandler = null;
    lastUpdateId = 0;
    sendOnly;
    fallbackHandler = null; // handler from connect() for new chatIds
    /** Cohere API key for voice transcription (STT) */
    cohereApiKey;
    ...
    constructor(config = {}) {
        ...
        this.token = config.botToken ?? _cfg.telegramBotToken() ?? "";
        ...
        if (hasPoll) {
            ...
            target.onMessage?.({
                ...
                meta: {
                    ...
                    poll_options: JSON.stringify((poll.options ?? []).map((o) => o.text)),
                    ...(msg.message_id != null ? { message_id: String(msg.message_id) } : {}),
                },
            });
            return;
    ...
    async handleFileMessage(target, msg, types) {
        try {
            let fileId;
            let fileName;
            let mime;
            ...
            if (types.hasVideoNote)
                meta.file_type = "video_note";
            if (types.hasAnimation)
                meta.file_type = "animation";
            if (types.hasSticker)
                meta.file_type = "sticker";
    ...
    startPolling() {
        ...
        (async () => { while (this.polling && gen === this.pollGeneration)
            await this.poll(); })();
    }
    // ── Webhook mode ──────────────────────────────────────────────────────
    /** Start the webhook HTTP server and register the URL with Telegram (idempotent). */
    async startWebhook() {
        if (this.webhookServer || this.sendOnly)
            return;
        ...
        await new Promise((resolve) => {
            this.webhookServer.listen(this.webhookPort, () => {
                process.stderr.write(`[telegram-transport] Webhook server listening on port ${this.webhookPort}${webhookPath}\n`);
                resolve();
            });
        });
    ...
    async connect(url, handler) {
        ...
        if (!this.botUsername && this.token) {
            try {
                const me = await tgCall(this.token, "getMe", {});
